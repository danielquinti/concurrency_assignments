...
struct args {
	...
	pthread_mutex_t *mutexarray;		  // One mutex for each element of the array
	pthread_mutex_t *countermutex;		  // Pointer to a common mutex
};
...
void *swap(void *ptr)
{
	...
	while((*args->iterations)>0) {
		pthread_mutex_lock(args->countermutex);	// Deny access the counter from other threads before performing the substraction
		(*args->iterations)--;
		...
		printf("Thread %d swapping positions %d (== %d) and %d (== %d)\n", 
			args->thread_num, i, args->buffer->data[i], j, args->buffer->data[j]);
      		pthread_mutex_lock(&args->mutexarray[MIN(i,j)]);
